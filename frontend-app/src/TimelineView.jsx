...
export default function TimelineView({
  ...
  const { parsedTasks, minD, maxD, days, months, sortedGroups } = useMemo(() => {
    ...
    if (rangePreset === 'this_month') {
      ...
      // 3 Bulan Kalender Penuh: Bulan Lalu, Bulan Ini, Bulan Depan
      computedMinD = new Date(today.getFullYear(), today.getMonth() - 1, 1);
      computedMaxD = new Date(today.getFullYear(), today.getMonth() + 2, 0);
    } else if (rangePreset === 'custom') {
      const cS = customStart ? new Date(customStart.replace(/-/g, '/')) : null;
      const cE = customEnd ? new Date(customEnd.replace(/-/g, '/')) : null;
      if (cS && cE && !isNaN(cS.getTime()) && !isNaN(cE.getTime()) && cS <= cE) {
  ...
  }, [filteredTasks, leaves, isUserAssigned, timelineDrag, groupBy, language, rangePreset, customStart, customEnd]);
  ...
  const scrollToToday = useCallback(() => {
    ...
    if (!scrollContainer || !minD || !maxD) return;
    const today = new Date();
    today.setHours(0, 0, 0, 0);
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xs border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col h-[calc(100vh-140px)] min-h-125">
      {/* Top Toolbar: Date Range Selector, Custom Date Picker, & Today Button */}
