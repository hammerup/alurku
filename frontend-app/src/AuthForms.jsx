...
export default function AuthForms({
  ...
  const emailHasDomain = regEmail.includes('@');
  const isDomainValid = isEmailDomainAllowed(regEmail);
  const showDomainError = emailHasDomain && !isDomainValid;
    ...
    <div className="min-h-screen w-full flex flex-col md:flex-row bg-[#f8f9ff] dark:bg-[#090D16] font-sans selection:bg-[#FACC15] selection:text-[#111E38] overflow-hidden">
      ...
      <section className="w-full md:w-1/2 lg:w-2/5 flex flex-col bg-[#f8f9ff] dark:bg-[#090D16] p-6 sm:p-12 lg:p-20 justify-between min-h-screen overflow-y-auto">
        ...
        <div className="w-full max-w-md mx-auto my-auto py-4">
          ...
          <header className="mb-8">
            ...
            <p className="text-neutral-500 dark:text-neutral-400 text-sm mt-2 font-medium">
              {isResetMode ? (
                ...
                tMsg("Registration is restricted to invited workspace members.", "Pendaftaran dibatasi khusus anggota workspace yang diundang.")
              ) : (
          ...
          {!isLoginMode && !isForgotMode && !isResetMode && (
            !policies.allow_public_signup ? (
              <div className="bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/20 rounded-2xl p-6 text-center space-y-5 animate-fade-up">
                ...
                <div className="space-y-2">
                  <h3 className="text-lg font-black text-[#111E38] dark:text-white">
                    {tMsg("Public Registration Closed", "Pendaftaran Akun Baru Ditutup")}
                  </h3>
                  <p className="text-xs text-neutral-600 dark:text-neutral-400 leading-relaxed max-w-sm mx-auto">
                    {tMsg(
                      "Self-registration is currently restricted by the organization administrator. If you have an invitation, please use the link sent to your email or sign in directly.",
                      "Pendaftaran mandiri saat ini sedang dinonaktifkan oleh administrator. Jika Anda memiliki undangan, silakan gunakan tautan yang dikirimkan ke email Anda atau langsung masuk."
                    )}
                  </p>
                </div>
                <button
                  ...
                  className="w-full py-3.5 bg-[#FACC15] hover:bg-[#EAB308] text-[#111E38] font-bold text-xs rounded-xl shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
                  <span>{tMsg("Sign In to Existing Account", "Masuk ke Akun Terdaftar")}</span>
              ...
              <form onSubmit={handleRegister} className="space-y-5 animate-fade-up">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="nama@email.com"
                        className={`w-full pl-12 pr-4 py-3.5 bg-white dark:bg-[#121B2D] border text-[#111E38] dark:text-white rounded-lg focus:ring-2 outline-none transition-all text-sm font-medium ${
                          showDomainError
                            ? 'border-red-400 focus:border-red-500 focus:ring-red-500/20'
                            : 'border-neutral-300 dark:border-neutral-700 focus:border-sky-500 focus:ring-sky-500/20'
                        }`}
                        required
                      />
                    </div>
                    {allowedDomainList.length > 0 && (
                      <div className="flex items-center gap-1.5 text-[11px] font-bold text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/40 px-2.5 py-1 rounded-md border border-sky-200 dark:border-sky-800/60 mt-1">
                        <span className="material-symbols-outlined text-[13px]">domain</span>
                        <span>{tMsg("Allowed domains:", "Domain diizinkan:")} @{allowedDomainList.join(', @')}</span>
                      </div>
                    )}
                    {showDomainError && (
                      <p className="text-[11px] font-bold text-red-500 mt-1 flex items-center gap-1">
                        <span className="material-symbols-outlined text-[13px]">cancel</span>
                        {tMsg("This email domain is not permitted to register.", "Domain email ini tidak diizinkan untuk mendaftar.")}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={!agreed || showDomainError}
                  className="w-full py-4 bg-[#FACC15] hover:bg-[#EAB308] text-[#111E38] disabled:opacity-50 disabled:cursor-not-allowed font-bold rounded-lg transition-all shadow-md flex items-center justify-center gap-2 group shadow-yellow-500/10 cursor-pointer"
                >
                ...
                policies.allow_public_signup ? (
                  <>
                    ...
                    <button
                      ...
                      className="font-extrabold text-sky-500 dark:text-sky-400 hover:text-sky-600 dark:hover:text-sky-350 hover:underline cursor-pointer"
                    >
                      {tMsg("Register Now", "Daftar Baru")}
                    </button>
                  </>
                ) : (
                  <span className="text-neutral-400 dark:text-neutral-500 font-medium normal-case tracking-normal">
                    {tMsg("Public registration is currently closed", "Pendaftaran akun baru saat ini ditutup")}
                  </span>
                )
              ) : (
                <>
