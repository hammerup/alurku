...
export default function AdminPage(props) {
  ...
  // Fetch latest users if needed on mount
  useEffect(() => {
    if (setAdminUsers) {
      axios.get('/api/admin/users').then((res) => setAdminUsers(res.data.users || [])).catch(() => {});
    }
  }, [setAdminUsers]);
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-[#F3F4F6] dark:bg-[#0d0f11]">
      {/* ── TOP PAGE HEADER ── */}
      <div className="bg-white/80 dark:bg-[#121B2D]/90 backdrop-blur-md border-b border-neutral-200/80 dark:border-neutral-800/80 px-6 py-4 shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button
            onClick={handleBackToWorkspace}
            className="p-2 rounded-xl text-slate-600 dark:text-neutral-300 hover:bg-neutral-200/60 dark:hover:bg-neutral-800 transition-colors flex items-center gap-1.5 text-xs font-bold cursor-pointer"
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-md bg-[#FACC15] text-[#111E38] text-[10px] font-black uppercase tracking-wider shadow-2xs">
                Super Admin
              </span>
            </div>
            <p className="text-xs text-neutral-500 dark:text-neutral-400 font-medium">
              {tMsg('Manage organization users, project directories, and system settings', 'Kelola pengguna organisasi, direktori proyek, dan pengaturan sistem')}
            </p>
          </div>
        </div>

        {/* Navigation Tabs */}
        <div className="flex items-center gap-1 bg-neutral-100 dark:bg-neutral-900/90 p-1 rounded-2xl border border-neutral-200 dark:border-neutral-800 self-start md:self-auto">
          <button
          ...
          {currentUser === 'admin' && (
            ...
            >
              ⚙️ {tMsg('System Config', 'Konfigurasi')}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 custom-scrollbar">
        {activeTab === 'users' ? (
          <div className="bg-white dark:bg-[#121B2D] border border-neutral-200 dark:border-neutral-800 rounded-3xl shadow-sm overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-neutral-200 dark:border-neutral-800 flex justify-between items-center bg-neutral-50/50 dark:bg-neutral-900/50 shrink-0 flex-wrap gap-4">
              <div className="flex items-center gap-4">
                <h3 className="font-bold text-[#111E38] dark:text-white text-sm uppercase tracking-wider">
                  {tMsg('User Directory', 'Direktori Pengguna')} ({filteredUsers.length})
                </h3>
                {selectedUsers.length > 0 && (
                  <div className="flex gap-2">
                    <button
                      ...
                      className="text-[10px] font-bold bg-emerald-500 text-white px-3 py-1.5 rounded-lg uppercase tracking-widest hover:bg-emerald-600 transition-colors shadow-sm cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="text-[10px] font-bold bg-red-500 text-white px-3 py-1.5 rounded-lg uppercase tracking-widest hover:bg-red-600 transition-colors shadow-sm cursor-pointer"
                    >
              ...
              <div className="relative w-full sm:w-auto">
                ...
                <input
                  ...
                  className="w-full sm:w-72 pl-8 pr-4 py-2 bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 text-black dark:text-white rounded-xl focus:border-indigo-500 outline-none text-xs font-medium transition-all"
                />
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-sm">
                <thead className="bg-neutral-100 dark:bg-neutral-800/80 text-neutral-500 dark:text-neutral-400 sticky top-0 z-10">
                  <tr>
                ...
                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                  {filteredUsers.map((u) => (
                    <tr key={u.username} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/40 transition-colors">
                          ...
                          <div className="flex justify-end items-center gap-2 flex-wrap">
                            {u.is_verified === 1 && u.username !== currentUser && (
                              <button
                                ...
                                className={`flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-lg transition-all border shadow-sm hover:-translate-y-0.5 cursor-pointer ${
                                  u.is_superadmin === 1
                            ...
                            {u.is_verified === 1 &&
                              (u.account_status === 'active' || u.account_status === 'offboarding') && (
                                <button
                                  ...
                                  className="flex items-center gap-1.5 text-xs font-bold text-cyan-700 bg-cyan-50 hover:bg-cyan-500 hover:text-white dark:bg-cyan-900/20 dark:text-cyan-400 px-3 py-1.5 rounded-lg transition-all border border-cyan-200 dark:border-cyan-800/50 shadow-sm hover:-translate-y-0.5 cursor-pointer"
                            ...
                            {u.is_verified === 1 &&
                              (u.account_status === 'active' || u.account_status === 'offboarding') && (
                                <button
                                  ...
                                  className="flex items-center gap-1.5 text-xs font-bold text-purple-700 bg-purple-50 hover:bg-purple-500 hover:text-white dark:bg-purple-900/20 dark:text-purple-400 px-3 py-1.5 rounded-lg transition-all border border-purple-200 dark:border-purple-800/50 shadow-sm hover:-translate-y-0.5 cursor-pointer"
                            ...
                            {u.is_verified === 1 && u.account_status === 'suspended' && (
                              <button
                                ...
                                className="flex items-center gap-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-500 hover:text-white dark:bg-emerald-900/30 dark:text-emerald-400 px-3 py-1.5 rounded-lg transition-all border border-emerald-200 dark:border-emerald-800/50 shadow-sm hover:-translate-y-0.5 cursor-pointer"
                            ...
                            {u.is_verified === 1 && u.account_status === 'pending_deletion' && (
                              <button
                                ...
                                className="flex items-center gap-1.5 text-xs font-bold text-blue-700 bg-blue-50 hover:bg-blue-500 hover:text-white dark:bg-blue-900/20 dark:text-blue-400 px-3 py-1.5 rounded-lg transition-all border border-blue-200 dark:border-blue-800/50 shadow-sm hover:-translate-y-0.5 cursor-pointer"
                            ...
                            {u.is_verified === 1 && u.account_status !== 'pending_deletion' ? (
                              <button
                                ...
                                className="flex items-center gap-1.5 text-xs font-bold text-red-700 bg-red-50 hover:bg-red-500 hover:text-white dark:bg-red-900/20 dark:text-red-400 px-3 py-1.5 rounded-lg transition-all border border-red-200 dark:border-red-800/50 shadow-sm hover:-translate-y-0.5 cursor-pointer"
                              ...
                              <>
                                <button
                                  ...
                                  className="flex items-center gap-1.5 text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-500 hover:text-white dark:bg-emerald-900/20 dark:text-emerald-400 px-3 py-1.5 rounded-lg transition-all border border-emerald-200 dark:border-emerald-800/50 shadow-sm hover:-translate-y-0.5 cursor-pointer"
          ...
          <div className="bg-white dark:bg-[#121B2D] border border-neutral-200 dark:border-neutral-800 rounded-3xl shadow-sm overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-neutral-200 dark:border-neutral-800 flex justify-between items-center bg-neutral-50/50 dark:bg-neutral-900/50 shrink-0 flex-wrap gap-4">
              <div className="flex items-center gap-4">
                <h3 className="font-bold text-[#111E38] dark:text-white text-sm uppercase tracking-wider">
                  {tMsg('Project Directory', 'Direktori Proyek')} ({filteredAdminBoards.length})
                </h3>
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto mt-2 sm:mt-0 flex-wrap sm:flex-nowrap">
                ...
                <select
                  ...
                  className="bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 text-xs font-bold p-2.5 rounded-xl outline-none cursor-pointer text-slate-700 dark:text-slate-300 shrink-0 [&>option]:bg-white dark:[&>option]:bg-neutral-900"
                >
                        ...
                        <td className="px-6 py-4 text-right whitespace-nowrap">
                          <div className="flex justify-end gap-2">
                            <button
                              ...
                              className="flex items-center gap-1.5 text-xs font-bold text-indigo-700 bg-indigo-50 hover:bg-indigo-500 hover:text-white dark:bg-indigo-900/20 dark:text-indigo-400 px-3 py-1.5 rounded-lg transition-all border border-indigo-200 dark:border-indigo-800/50 shadow-sm hover:-translate-y-0.5 cursor-pointer"
          ...
          <div className="max-w-4xl mx-auto py-2">
            <form onSubmit={handleConfigSubmit} className="space-y-8">
              ...
              <div className="bg-white dark:bg-[#121B2D] border border-neutral-200 dark:border-neutral-800 rounded-3xl p-6 md:p-8 shadow-sm">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-neutral-50 dark:bg-black/50 border border-neutral-200 dark:border-neutral-700 rounded-xl p-3 text-sm font-medium outline-none focus:border-indigo-500 transition-colors pr-10 text-black dark:text-white"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-neutral-50 dark:bg-black/50 border border-neutral-200 dark:border-neutral-700 rounded-xl p-3 text-sm font-medium outline-none focus:border-indigo-500 transition-colors text-black dark:text-white"
              ...
              <div className="flex justify-end pt-4">
                <button
                  ...
                  className="bg-[#111E38] hover:bg-neutral-800 text-white dark:bg-[#FACC15] dark:text-[#111E38] font-black px-8 py-4 rounded-2xl text-xs uppercase tracking-widest shadow-lg hover:-translate-y-0.5 transition-all disabled:opacity-50 cursor-pointer"
                >
                  {isConfigLoading
      ...
      {/* ── MODALS (DELETE CHOICE, BULK ACTION, TRANSFER, SUDO) ── */}
      ...
      {showDeleteChoice && deleteChoiceUser && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-100 flex flex-col items-center justify-center p-4">
          <div className="bg-white dark:bg-neutral-950 p-6 sm:p-10 w-full max-w-md border border-neutral-200 dark:border-neutral-800 shadow-2xl rounded-3xl text-center flex flex-col items-center">
      ...
      {/* User Processing Confirmation Modal */}
      {userToProcess &&
        (() => {
          let icon = '⚠️';
      ...
      {/* Sudo Password Security Gate Modal */}
      {showSudoModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-100 flex flex-col items-center justify-center p-6">
          <div className="bg-white dark:bg-neutral-950 p-6 sm:p-10 w-full max-w-md border border-neutral-200 dark:border-neutral-800 shadow-2xl rounded-3xl text-center flex flex-col items-center">
            <div className="w-20 h-20 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500 rounded-full flex items-center justify-center mx-auto mb-6 text-4xl shadow-sm border border-indigo-200 dark:border-indigo-800">
              🔐
            </div>
            ...
            <p className="text-neutral-500 dark:text-neutral-400 max-w-md text-center text-xs leading-relaxed mb-6">
              {tMsg(
            ...
            <form onSubmit={handleSudoSubmit} className="w-full mb-2">
              <div className="relative mb-6">
                <input
                  ...
                  className="w-full p-4 bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 text-black dark:text-white rounded-2xl focus:border-indigo-500 outline-none text-sm font-bold transition-all pr-12"
              ...
              <div className="flex gap-4 w-full">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-4 rounded-full font-bold text-white transition-all text-xs uppercase tracking-widest shadow-md bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-0.5"
                >
